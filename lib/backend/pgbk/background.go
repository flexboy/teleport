...
func (b *Backend) runChangeFeed(ctx context.Context) error {
	...
	if _, err := conn.Exec(ctx,
		fmt.Sprintf("ALTER ROLE %s REPLICATION", pgx.Identifier{poolConfig.ConnConfig.User}.Sanitize()),
...
FROM (SELECT data::jsonb AS data FROM pg_logical_slot_get_changes($1, NULL, $2,
