...
func NewTeleport(cfg *servicecfg.Config) (*TeleportProcess, error) {
	...
	if len(process.Config.AuthServerAddresses()) != 0 && process.Config.AuthServerAddresses()[0].Port(0) == 0 {
		...
		if err == nil {
			process.Config.SetAuthServerAddress(utilsaddr.FromAddr(listener.Addr()))
		}
	}
...
func (process *TeleportProcess) proxyPublicAddr() utilsaddr.NetAddr {
...
func (process *TeleportProcess) getAdditionalPrincipals(role types.SystemRole) ([]string, []string, error) {
	...
	case types.RoleProxy:
		addrs = append(process.Config.Proxy.PublicAddrs,
			...
			utilsaddr.NetAddr{Addr: string(teleport.PrincipalLocalhost)},
			utilsaddr.NetAddr{Addr: string(teleport.PrincipalLoopbackV4)},
			utilsaddr.NetAddr{Addr: string(teleport.PrincipalLoopbackV6)},
			utilsaddr.NetAddr{Addr: reversetunnelclient.LocalKubernetes},
		)
		...
		if process.Config.Proxy.Kube.Enabled {
			for _, publicAddr := range utilsaddr.JoinAddrSlices(process.Config.Proxy.PublicAddrs, process.Config.Proxy.Kube.PublicAddrs) {
	...
	case types.RoleNode:
		...
		if process.Config.AdvertiseIP != "" {
			advertiseIP, err := utilsaddr.ParseAddr(process.Config.AdvertiseIP)
			if err != nil {
				return nil, nil, trace.Wrap(err)
			}
	...
	case types.RoleWindowsDesktop:
		addrs = append(addrs,
			...
			utilsaddr.NetAddr{Addr: string(teleport.PrincipalLoopbackV4)},
			utilsaddr.NetAddr{Addr: string(teleport.PrincipalLoopbackV6)},
			utilsaddr.NetAddr{Addr: reversetunnelclient.LocalWindowsDesktop},
			utilsaddr.NetAddr{Addr: desktop.WildcardServiceDNS},
		)
	...
	if process.Config.OpenSSH.Enabled {
		for _, a := range process.Config.OpenSSH.AdditionalPrincipals {
			addr, err := utilsaddr.ParseAddr(a)
			if err != nil {
				return nil, nil, trace.Wrap(err)
			}
...
func (process *TeleportProcess) initProxyEndpoint(conn *Connector) error {
	...
	rcWatcher, err := reversetunnel.NewRemoteClusterTunnelManager(reversetunnel.RemoteClusterTunnelManagerConfig{
		...
		KubeDialAddr:        utilsaddr.DialAddrFromListenAddr(kubeDialAddr(cfg.Proxy, clusterNetworkConfig.GetProxyListenerMode())),
	...
	if !cfg.Proxy.DisableTLS && !cfg.Proxy.DisableALPNSNIListener && listeners.web != nil {
		authDialerService := alpnproxyauth.NewAuthProxyDialerService(
			tsrv,
			clusterName,
			utilsaddr.NetAddrsToStrings(process.Config.AuthServerAddresses()),
			proxySigner,
			process.log,
...
func kubeDialAddr(config servicecfg.ProxyConfig, mode types.ProxyListenerMode) utilsaddr.NetAddr {
...
func (process *TeleportProcess) singleProcessMode(mode types.ProxyListenerMode) (*utilsaddr.NetAddr, bool) {
	...
	if len(process.Config.Proxy.TunnelPublicAddrs) == 0 {
		addr, err := utilsaddr.ParseHostPortAddr(string(teleport.PrincipalLocalhost), defaults.SSHProxyTunnelListenPort)
		if err != nil {
			return nil, false
		}
		return addr, true
